...
sample_data <- genus_summary %>%
    ...
    pivot_longer(
        cols = c(report_merged, report_PE),
        names_to = "Sample",
...
p_kraken <- ggplot(sample_data, aes(x = Percentage, y = Genus, fill = Phylum)) +
    ...
    labs(
        title = "A) Kraken2: Top Genera by Sample Type",
        x = "Percentage (%)",
        y = "Genus",
        fill = "Phylum"
    ) +
...
p_ecmsd_prop <- ggplot(ecmsd_top20, aes(
    x = reorder(genus, Percentage),
    y = Percentage,
    fill = genus
)) +
    geom_col() +
    ...
    coord_flip() +
    labs(
        title = "B) ECMSD: Top 20 Genera by Proportion",
        x = "Genus",
        y = "Percentage (%)"
    ) +
    theme_bw(base_size = 18) +
...
p_ecmsd_lengths <- ggplot(ecmsd_lengths_top10, aes(
    x = Length,
    y = TotalReads,
    color = genus
)) +
    ...
    labs(
        title = "C) ECMSD: Read Length Distribution (Top 10 Genera)",
        x = "Read Length (bp)",
        y = "Total Reads"
    ) +
    theme_bw(base_size = 18) +
