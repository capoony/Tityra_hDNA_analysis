...
data <- data %>%
  ...
  pivot_longer(cols = c(report_PE, report_merged), 
               names_to = "Sample", 
...
p1 <- ggplot(genus_data, aes(x = reorder(Genus, Mean_Percentage), 
                              y = Mean_Percentage, 
                              fill = Phylum)) +
  geom_bar(stat = "identity", color = "black", linewidth = 0.2) +
  scale_fill_manual(values = phylum_colors) +
  coord_flip() +
  labs(title = "Top 20 Genera in Kraken2 Classification",
...
sample_data <- genus_summary %>%
  ...
  mutate(Sample = factor(Sample, levels = c("merged", "PE"), 
                        labels = c("Merged", "Paired-End")))

# Create ordered factor for consistent y-axis (descending order - highest at top)
genus_order <- top_genera_list %>% arrange(Combined_Total) %>% pull(Genus)
sample_data <- sample_data %>%
  mutate(Genus = factor(Genus, levels = genus_order))

p2 <- ggplot(sample_data, aes(x = Total_Percentage, 
                               y = Genus, 
                               fill = Phylum)) +
  geom_bar(stat = "identity", color = "black", linewidth = 0.2) +
  scale_fill_manual(values = phylum_colors) +
  facet_wrap(~Sample, scales = "fixed") +
  labs(title = "Top Genera by Sample Type",
