...
7. **Phylogenetic Analysis with BUSCO Genes**
   - This comprehensive phylogenetic analysis uses BUSCO genes identified from the de novo assembly and reference genomes to construct evolutionary relationships.
   - Outputs: Multiple sequence alignments, phylogenetic trees, and bootstrap support values.
   - Interpretation: The analysis identifies single-copy orthologs present across multiple bird genomes using the BUSCO vertebrata_odb10 and aves_odb10 databases. BUSCO genes that are complete in all 14 analyzed genomes are concatenated and aligned using MAFFT. The resulting supermatrix is used for maximum likelihood phylogenetic reconstruction with RAxML using the PROTGAMMAWAG model and bootstrap resampling for statistical support.
...
- The consensus mitochondrial genome of *Tityra leucura* was successfully reconstructed from reads mapped against the *Pachyramphus minor* reference genome. Combined with BUSCO genes recovered from the de novo assembly, these provide a solid foundation for phylogenetic analyses.

- The phylogenetic analysis successfully places *Tityra leucura* within the broader context of avian evolution using single-copy orthologous genes. This represents one of the first molecular phylogenetic placements of this species using genomic data recovered from historical specimens.
