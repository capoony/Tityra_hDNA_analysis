...
mkdir -p ${WD}/results/phylogeny/Oxyruncus_cristatus_BUSCO/consensus_pmin
...
echo "Region: $REGION"
OUT=${WD}/results/phylogeny/Oxyruncus_cristatus_BUSCO/consensus_pmin/Tityra_leucura_${gene}.fasta
...
mkdir -p ${WD}/results/phylogeny_DNA_Ocrist_pminGenes/concatenated

cd ${WD}/results/phylogeny_DNA_Ocrist_pminGenes/BUSCO
...
mkdir -p ${WD}/results/phylogeny_DNA_Ocrist_pminGenes/concatenated/busco_dna
...
while IFS=$"," read -r Name; do
    ...
    while IFS=$"," read -r gene; do
        ...
        cp ${WD}/results/phylogeny/BUSCO/${Name}/run_aves_odb10/busco_sequences/single_copy_busco_sequences/${gene}.fna \
            ${WD}/results/phylogeny_DNA_Ocrist_pminGenes/concatenated/busco_dna/${Name}_${gene}
        sed -i 's/^>/>'${Name}'|/g' ${WD}/results/phylogeny_DNA_Ocrist_pminGenes/concatenated/busco_dna/${Name}_${gene}
    done <${WD}/results/phylogeny/Pachyramphus_minor_BUSCO/Tityra_leucura_Pachyramphus_minor_PE.genes.txt

done <${WD}/results/phylogeny/data/genomes.names

### concatenate and reduce to shared genes, what a beautiful code!!!
mkdir -p ${WD}/results/phylogeny_DNA_Ocrist_pminGenes/prealigned

while read gene; do
    ## add remaining species from BUSCO results
    cat ${WD}/results/phylogeny_DNA_Ocrist_pminGenes/concatenated/busco_dna/*_${gene}* \
        >>${WD}/results/phylogeny_DNA_Ocrist_pminGenes/prealigned/${gene}_dna.fasta
    ## add consensus sequences from Tityra mapped to Pachyramphus minor BUSCO genes
    cat /media/inter/mkapun/projects/Tityra/results/phylogeny/Oxyruncus_cristatus_BUSCO/consensus_pmin/Tityra_leucura_${gene}.fasta \
        >>${WD}/results/phylogeny_DNA_Ocrist_pminGenes/prealigned/${gene}_dna.fasta
done <${WD}/results/phylogeny/Pachyramphus_minor_BUSCO/Tityra_leucura_Pachyramphus_minor_PE.genes.txt

mkdir ${WD}/results/phylogeny_DNA_Ocrist_pminGenes/mafft

conda activate mafft-7.487
for i in ${WD}/results/phylogeny_DNA_Ocrist_pminGenes/prealigned/*_dna.fasta; do
    ...
    mafft \
        ...
        --auto \
        ${i} \
        >${WD}/results/phylogeny_DNA_Ocrist_pminGenes/mafft/${ID}_aln.fasta
...
for i in ${WD}/results/phylogeny_DNA_Ocrist_pminGenes/prealigned/*_dna.fasta; do
    ...
    python /media/inter/mkapun/projects/TorpedoDeNovo/scripts/fixIDAfterMafft.py \
        --Alignment ${WD}/results/phylogeny_DNA_Ocrist_pminGenes/mafft/${ID}_aln.fasta \
        --input ${i} \
        >${WD}/results/phylogeny_DNA_Ocrist_pminGenes/mafft/${ID}_aln_fixed.fasta

    rm -f ${WD}/results/phylogeny_DNA_Ocrist_pminGenes/mafft/${ID}_aln.fasta

done

## make phylogeny

mkdir ${WD}/results/phylogeny_DNA_Ocrist_pminGenes/phylogeny

python /media/inter/mkapun/projects/TorpedoDeNovo/scripts/proteins2genome.py \
    --input ${WD}/results/phylogeny_DNA_Ocrist_pminGenes/mafft \
    >${WD}/results/phylogeny_DNA_Ocrist_pminGenes/phylogeny/alignment.fa

module load Phylogeny/RAxML-2.8.10

## make new directory

cd ${WD}/results/phylogeny_DNA_Ocrist_pminGenes/phylogeny
...
raxmlHPC-PTHREADS-SSE3 \
    ...
    -p 772374015 \
    -n Tityra \
    -s ${WD}/results/phylogeny_DNA_Ocrist_pminGenes/phylogeny/alignment.fa \
...
tree<-read.tree('${WD}/results/phylogeny_DNA_Ocrist_pminGenes/phylogeny/RAxML_bipartitions.FINAL')
...
## export tree
ggsave(filename='${WD}/results/phylogeny_DNA_Ocrist_pminGenes/phylogeny/Tityra_BUSCO.pdf',
  PLOT.tree)
ggsave(filename='${WD}/results/phylogeny_DNA_Ocrist_pminGenes/phylogeny/Tityra_BUSCO.png',
  PLOT.tree)
"""
