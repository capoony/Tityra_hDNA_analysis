...
###############################################################################
# Combine Mitochondrial Gene Sequences
# Description: Join gene-specific sequences from downloaded data and add
#              Tityra sequences from MitoFinder results
###############################################################################
...
TITYRA_GENES_FILE="${WD}/results/mitofinder_rawreads/Tityra_leucura/Tityra_leucura/Tityra_leucura_MitoFinder_megahit_mitfi_Final_Results/Tityra_leucura_mtDNA_contig_1_genes_NT.fasta"

# Output directory
OUTPUT_DIR="${WD}/results/combined_mito_genes"
mkdir -p "${OUTPUT_DIR}"

# Log file
LOG="${WD}/logs/combine_mito_genes.log"
mkdir -p "${WD}/logs"

exec > >(tee -a "${LOG}") 2>&1

echo "==================================================================="
echo "Combining Mitochondrial Gene Sequences"
echo "Started: $(date)"
echo "==================================================================="
